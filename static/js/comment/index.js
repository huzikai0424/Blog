...
class Comment extends Component{
	...
	render(){
		...
		let result = this.state.commentList.map((obj,index)=>{
				...
				if (item.pid == obj.id){
					...
					let arr = (
						<div className="child-comment-main" data-commentid={item.id} key={key}>
							...
							<a href={item.website} target="_blank" rel="noopener noreferrer"><span>{item.nickname}</span></a>
			...
			let chilidComments = ()=>{
				let arr = chilidCommentsArr.map((obj) => obj )
				if (arr.length){
					return (
						...
						<div className="author-info">
							...
							<p>
								<Tooltip placement="topLeft" title={`${ua.browser.name} ${ua.browser.major} / ${ua.os.name} ${ua.os.version}`}>
									<a href={obj.website} target="_blank" rel="noopener noreferrer"><span className="ua author-name">{obj.nickname}</span></a>
								</Tooltip>
				...
				<Form onSubmit={this.handleSubmit}>
					<Form.Item>
						{getFieldDecorator('comment', {
							...
							<TextArea placeholder={this.state.placeholder} rows={6} tabIndex="1" />
						)}
					</Form.Item>
					<div id="author-info">
						<Form.Item
